...
suite('Functional Tests', () => {
  const invalidStringChar = '?.5..2.84..63.12.7.2..5.....9..1....8.2.3674.3.7.2..9.47...8..1..16....926914.37.'
  const invalidStringLen = '.';
  const conflictingString = '125..2.84..63.12.7.2..5.....9..1....8.2.3674.3.7.2..9.47...8..1..16....926914.37.'
  test('Test /api/solve POST with valid puzzle string', (done) => {
    chai
      .request(server)
      .post('/api/solve')
      .set('content-type', 'application/json')
      .send({puzzle: puzzles[1][0]})
      .end((err, res) => {
        if(err){
          done(err);
        } else {
          assert.equal(res.status, 200);
          assert.property(res.body, 'solution', 'Return value must contain property solution');
    ...
    chai
      ...
      .end((err, res) => {
        if(err){
          ...
          assert.equal(res.status, 200);
          assert.property(res.body, 'error', 'Return value must contain property error');
    ...
    chai
      ...
      .end((err, res) => {
        if(err){
          ...
          assert.equal(res.status, 200);
          assert.property(res.body, 'valid', 'Return value must contain property valid');
    ...
    chai
      ...
      .end((err, res) => {
        if(err){
          ...
          assert.equal(res.status, 200);
          assert.property(res.body, 'valid', 'Return value must contain property valid');
          assert.property(res.body, 'conflict', 'Return value must contain property conflict');
